...
#https://www.youtube.com/playlist?list=PLgcjCT1pz7y1uOCDJtoYe3X7xB9dMDAEO
    ...
    def print_video(self):
        while True:
            try:
                numOFresults = int(input("How many results you want: "))
            except ValueError:
                print('Sorry, please enter the number of results in numbers.!')
                continue
            vidORlist = input("Search for a video or playlist (v/p): ")
            if vidORlist == 'v':
...
{'-' * 30}{int(list['index'])+1}{'-' * 30}
    ...
    def selectQuality(self,quality):
        ...
        if len(Q_) == 0:
            print("Sorry, this quality is not available..! (Choose agin.!)")
            self.quality = None
        elif len(Q_) >= 1:
            video = Q_.first()
